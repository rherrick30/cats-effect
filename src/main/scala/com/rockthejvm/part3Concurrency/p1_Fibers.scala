...
object p1_Fibers extends IOApp.Simple {
  ...
  def testCancel() = {
    ...
    val taskWithCancellationHandler = task.onCancel(IO("dude I was totally cancelled!").debug.void)
    for {
  ...
  import cats.effect.implicits.*
    ...
    result <- fib.join // an effect which waits for the fiber to terminate
      ...
      case Canceled() => IO.raiseError(new RuntimeException("a proper fail")).unsafeRunSync()
  ...
  def testEx1() = {
    val aComputation = IO("Starting1").debug >> IO.sleep(1.second) >> IO("complete1").debug >> IO(42)
    processResultsFromFiber(aComputation).void
  }

  def testEx1mine() = {
    val aComputation2 = IO("Starting2").debug >> IO.sleep(1.second) >> IO("complete2").debug >> IO(78)
...
//      } yield (a, b)
      ...
      case (Canceled(), _) | (_, Canceled()) => IO.raiseError(new RuntimeException("a proper cancel"))
    }
  ...
  def timeout[A](ioa: IO[A], duration: FiniteDuration) : IO[A] =  {
    ...
    result.flatMap {
      case Succeeded(fa) => fa
      case Errored(e) => IO.raiseError(e)
      case Canceled() => IO.raiseError(new RuntimeException("cancel of computation"))
    }
  }

  def testEx3() = {
    val aComputation = IO("Starting1").debug >> IO.sleep(2.second) >> IO("complete1").debug >> IO(42)
